...
function userEvents () {
  setInterval (function () {
    ...
    .done(function( json ) {
      ...
      let toAdd = json.length - lastLength;
      json = json.slice(json.length-toAdd,json.length)
      console.log(toAdd)
      ...
      for (let i of json) {
        ...
        marker.addListener('click', function() {
          ...
          infoWindow = new google.maps.InfoWindow({
            content: "<h1>" + this.data.name + "</h1><p>" + this.data.Description + "</p>"
          });
      ...
      geocoder.geocode({'address': i.loc}, function(results, status) {
        if (status === 'OK') {
          ...
          marker.addListener('click', function() {
            if (infoWindow != null) infoWindow.close()
            infoWindow = new google.maps.InfoWindow({
              content: "<h1>" + this.data.name + "</h1><p>" + this.data.desc + "</p>"
            });
            infoWindow.open(map, this);
          });
          eventMarkers.push(marker)
  ...
  if (data.type == "loc"){
    ...
    geocoder.geocode({'address': events[data.id].loc}, function(results, status) {
      if (status === 'OK') {
        ...
        if (infoWindow != null) infoWindow.close()
        infoWindow = new google.maps.InfoWindow({
          content: "<h1>" + events[data.id].name + "</h1><p>" + events[data.id].desc + "</p>"
        });
        infoWindow.open(map, events[data.id].marker);
      }else{
        console.log('Geocode was not successful for the following reason: ' + status);
      }
    });
    ...
    for (let j of places) {
      if (events[data.id].location == j.code) {
        ...
        if (infoWindow != null) infoWindow.close()
        infoWindow = new google.maps.InfoWindow({
          content: "<h1>" + events[data.id].name + "</h1><p>" + events[data.id].Description + "</p>"
        });
        infoWindow.open(map, events[data.id].marker);
        break;
      }
